class ALSModelTrainerMFCC:
    def __init__(self, train_data_path, test_data_path,  mfcc_feature_path, classification_type, exp_name):
        self.train_data_path = train_data_path
        self.test_data_path = test_data_path
        self.classification_type = classification_type
        self.exp_name = exp_name
        self.mfcc_feature_path = mfcc_feature_path
        self.fold_metrics = []
    ...
    def process_data(self, i):
        ...
        mfcc_features_data = pd.read_csv(self.mfcc_feature_path)
        ...
        train_data = mfcc_features_data[train_files['Audio_file_name'].unique()]
        train_labels = [train_files.loc[train_files['Audio_file_name'] == x, 'Severity'].values[0] for x in train_data.columns]
        test_data = mfcc_features_data[test_files['Audio_file_name'].unique()]
    ...
    def train_model(self, train_loader, test_loader, train_data_tensor, test_data_tensor, train_labels_tensor, test_labels_tensor,exp_name):
        model = SimpleDNN_drop(input_shape=train_data_tensor.shape[-1], num_classes=torch.unique(train_labels_tensor).shape[0])
