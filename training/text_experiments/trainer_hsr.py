...
class ALSModelTrainerHSR:
    def __init__(self, train_data_path, test_data_path, labse_model, tokenizer, classification_type, exp_name,hsr_type,feature_type,keyword):
        ...
        self.fold_metrics = []
        self.hsr_type = hsr_type
        self.combined_data_flag = True
        self.feature_type = feature_type
        self.keyword = keyword

    @staticmethod
    def create_audio_transcript_dict(df,mfcc_train_data):
      transcript_dict = {}
      mfcc_dict = {}
      labels = []
      grouped = df.groupby(['Audio_file_name', 'Type'])
      for (audio_file, audio_type), group in grouped:
          key = f"{audio_file}"
          mfcc_feat = mfcc_train_data[audio_file]
          if key not in transcript_dict:
              transcript_dict[key] = []
              mfcc_dict[key] = mfcc_feat
              labels.append(group['Severity'].tolist()[0])

          transcript_dict[key].extend(group['Transcript'].tolist())

      return list(transcript_dict.values()), list(mfcc_dict.values()), labels
    @staticmethod
    # Function to remove unwanted words
    def remove_unwanted_words(sentence):
        words_to_remove = ['<CLICK>', '<PAUSE>', '<LONGSIL>', '<GARBAGE>']
        pattern = '|'.join(map(re.escape, words_to_remove))
        return re.sub(pattern, '', sentence)
    @staticmethod
    def replace_words(row):
      # Define language-specific replacement dictionaries
      language = row['Language']
      sentence = row['Transcript']

      if language == "Telugu":
          replacement_dict = {"<CLICK>": '<క్లిక్>', '<GARBAGE>': '<గార్బేజ్>', '<PAUSE>': '<పాస్>', '<LONGSIL>': '<లాంగ్సిల్>'}
      elif language == "Tamil":
          replacement_dict = {"<CLICK>": '<கிளிக்>', '<GARBAGE>': '<கார்பேஜ்>', '<PAUSE>': '<பாஸ்>', '<LONGSIL>': '<லோங்சில்>'}
      elif language == "Hindi":
          replacement_dict = {"<CLICK>": '<क्लिक>', '<GARBAGE>': '<गार्बेज>', '<PAUSE>': '<पॉज>', '<LONGSIL>': '<लोंगसील>'}
      elif language == "Kannada":
          replacement_dict = {"<CLICK>": '<ಕ್ಲಿಕ್>', '<GARBAGE>':'<ಗಾರ್ಬೇಜ್>', '<PAUSE>': '<ಪೌಜ್>', '<LONGSIL>': '<ಲೋಂಗ್ಸಿಲ್>'}
      elif language == "Bengali":
          replacement_dict = {"<CLICK>": '<ক্লিক>', '<GARBAGE>': '<গার্বেজ>', '<PAUSE>': '<পজ>', '<LONGSIL>': '<লংসিল>'}
      else:
          replacement_dict = {}

      # Replace words in the sentence based on the replacement dictionary
    ...
    def extract_and_normalize_embeddings_batch(self, text_data, model, tokenizer, max_length=128, batch_size=32, device='cuda',combined_data_flag=True):
        """
    ...
    def LASER_extract_and_normalize_embeddings_batch(self, text_data, model, tokenizer, max_length=128, batch_size=32, device='cuda',combined_data_flag=True):
        """
        Extract embeddings from a list of text and normalize them.
        Optimized for batch processing.
        """
    ...
    def process_data(self, i):
        ...
        if self.hsr_type == 'all':
           ...
           if self.keyword == 'remove_keywords':
            train_files['Transcript'] = train_files['Transcript'].apply(self.remove_unwanted_words)
           if self.keyword == 'transliterate':
            train_files['Transcript'] = train_files.apply(self.replace_words, axis=1)
           ...
           if self.keyword == 'remove_keywords':
            test_files['Transcript'] = test_files['Transcript'].apply(self.remove_unwanted_words)
           if self.keyword == 'transliterate':
            test_files['Transcript'] = test_files.apply(self.replace_words, axis=1)

           mfcc_features_data = pd.read_csv(f'/content/drive/MyDrive/Detection of ALS/MFCC_stats_features.csv')
           mfcc_train_data = mfcc_features_data[train_files['Audio_file_name'].unique()]
           mfcc_test_data = mfcc_features_data[test_files['Audio_file_name'].unique()]


           train_data, _ , train_labels= self.create_audio_transcript_dict(train_files, mfcc_train_data)
           test_data, _ , test_labels = self.create_audio_transcript_dict(test_files, mfcc_test_data)

        else:
          train_files = pd.read_excel(self.train_data_path, sheet_name=f'Combinations{i}').sort_values(by='Audio_file_name')
          train_files = train_files[train_files['Type']==self.hsr_type]
          train_files['Transcript'] = train_files['Transcript'].fillna('')

          test_files = pd.read_excel(self.test_data_path, sheet_name=f'Combinations{i}').sort_values(by='Audio_file_name')
          test_files = test_files[test_files['Type']==self.hsr_type]
          test_files['Transcript'] = test_files['Transcript'].fillna('')
